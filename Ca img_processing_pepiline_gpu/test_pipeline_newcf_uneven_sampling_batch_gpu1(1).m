...
for batchi = [22 19 20 21]
    ...
    load([opt.savepath 'act.mat']);
    load([opt.savepath 'env.mat']);
            ...
            spmd(poolsize)
                ...
                for ii = 1:length(ti_in_this_thread)
                    ...
                    for zi = 1:depth
                        ...
                        if ~ismember(ti,opt.ind_stand_volet_total(:)) && ((zi<opt.ind_beg_step)||(zi>opt.ind_end_step))
                            backth(:,ii,zi) = nan;
                            backthcf(ii,zi) = nan;
                            for ppi = spvzind{zi}
