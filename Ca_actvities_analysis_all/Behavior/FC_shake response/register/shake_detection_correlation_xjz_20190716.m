...
%ģ��Ϊÿ��trial��һ��
% parfor ii=framenum
%     sourcenum=fix((ii-1)/frame.per_cycle)*frame.per_cycle+1;%%��ii=n*frame.per_cycleʱ��trial����Ĳ���?
...
x=delta_r_bef1;figure,plot(x);
    ...
    yt = -1*ones(size(xt));
    txt = num2cell(1:trial.total);%������Ҫʹ��cell��ʽ���У����ʹ��chr��ʽ�Ļ�������Ҫת�ó�������
    text(xt,yt,txt)
    hold off
fluctuation=input('input max thr of baseline(can be larger than it)\n');
[y_3sd,startpoint_sd,thr_sd]=findpeak_use3sd(x,frame.bin,fluctuation,frame.per_cycle);
...
% bin=[30 30 30];
% a=[20 2 5];
% [y_3sd,startpoint_sd,thr_sd]=findpeak_use3sd_seg(x,bin,a,trials,frame.per_cycle);%�ֶΣ��ֳ�HAB,ACQ,TEST,�����ڻ��߱仯�����?

% trial=[0 33 57 81]; %after201810
% x=delta_r_bef1;
% bin=[32 32 32];
% fluctuation=[2 2 2];
% [y,startpoint_sd,thr]=findpeak_use3sd_seg(x,[32 32 32],fluctuation,trials,frame.per_cycle);%�ֶΣ��ֳ�HAB,ACQ,TEST,�����ڻ��߱仯�����?
% [y_t,startpoint_t,thr]=findpeak_usetendency(x);

%reshape
re_startpoint_sd(:,1)=fix(startpoint_sd/frame.per_cycle)+1;
re_startpoint_sd(:,2)=mod(startpoint_sd,frame.per_cycle);
...
%      're_startpoint_sd','re_startpoint_t','-v7.3');
  save([outputName '\shake.mat'],'trial','frame','delta_r_bef1','delta_r_bef12','delta_c_bef1','delta_c_bef12','pp_bef1',...
     'y_3sd','startpoint_sd','re_startpoint_sd');