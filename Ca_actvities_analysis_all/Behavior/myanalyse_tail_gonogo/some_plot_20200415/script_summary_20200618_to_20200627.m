...
unchanged=[7 9 16 23];learner=[14 17 21 24 9];
more_active=[17 21 24];nonlearner=[11 13 ];
p_a_p=[2 12 13];not_sure=[15 22 ];
passive=[11];reversed=[2 18 ];
a_p_a=[15 18 22];hab_CR=[7 12 16 23];

type=[17 21 24 9, reversed,nonlearner,not_sure,hab_CR];
...
%b= CR_trial{3}(:,type(ind));b(find(b>=1))=1; %bout number
...
for ii=1:5
    switch ii
        case 1
            type=learner;%type=unchanged;
        case 2
            type=reversed;%type=more_active;
        case 3
            type=nonlearner;%type=a_p_a;
        case 4
            type=not_sure;%type=p_a_p;
        case 5
            type=hab_CR;%type=passive;
    end
    b=Data.M_num_spon(1:9,type);
    bb=(b-repmat(b(1,:),9,1))./(b+repmat(b(1,:),9,1));
    B(:,(ii-1)*5+1:(ii-1)*5+size(b,2))=b;
