function [posarray, thetaarray] = find_tail(frame, backim, pos0, theta0, npoints, r, params)
% [posarray, thetaarray] = find_tail(frame, backim, pos0, theta0, npoints, r, params)
%
...
%% use original image as engim for tail base
engim = frame;
for pti = 2:prepoints
    w = segwidth(pti);
    baseth = thetaarray(pti-1);
    %thetas = baseth-movescope:movestep:baseth+movescope;
    if pti == 2  % restrain the first point
    ...
    [x, y, theta] = find_next(engim, thetas, x0, y0, w, r);
    
    posarray(pti,:) = [x y];
    thetaarray(pti) = theta;
    x0 = x; y0 = y;
end

%% engim for tail
engim = frame - backim;
for pti = prepoints+1:npoints
    w = segwidth(pti);
    baseth = thetaarray(pti-1);
    thetas = baseth-movescope:movestep:baseth+movescope;
    ...
    function [x, y, theta] = find_next(engim, thetas, x0, y0, w, r)
