...
%% load images
[fpaths, nframes] = ask_for_images_normal;  % caution: image formats/names dependent

%% options for movement detections
dirpath = fileparts(fpaths{1});
[~, dirname] = fileparts(dirpath);
savepath = checkpath([dirpath '/../' dirname '_find_tail/']); %% /.. stands for parental folder
% savepath = checkpath('Z:/data/20150625/rec5338806_sig/tail_new/');

npoints = 4;   % npoints-1 segments

winsize = 250;  % temporal window for movement detection, better be odd
boxwidth = 5;  % spatial zone for movement detection
...
params.prolong_factor = 1.2;
params.turn_scope = 45;
params.turn_step = 1;
params.max_width = 2;   % radius width used in roi_line_width
params.min_width = 0.5;
...
% this estimation is only for head fixed fish tail on ls setup
%movlabel = bwlabel(imclose(moving(:)', ones(1, winsize)));

moving = true(1, nframes);
movlabel = bwlabel(moving);
...
%
%bouts = [1.44, 1.63, 2.80, 2.90, 4.12, 4.28, 5.22, 5.40, 7.20, 7.32, 7.77, 7.80];
allbouts = dlmread('z:/data freemoving/20160322/bouts.txt');
expath = fpaths{1};
sampleid = str2double(expath(65:66));
cutid = str2double(expath(72));
boutid = (sampleid - 1) * 4 + cutid;
bouts = allbouts(boutid, :);
bouts = nonzeros(bouts);

moving = false(1, nframes);
for tti = 1:2:length(bouts)
    moving(round(t2f(bouts(tti), 1)):round(t2f(bouts(tti+1), 1))) = true;
end
moving = moving(1:nframes);

moving = imdilate(moving, strel('disk', 5));
% moving = true(1, nframes);
movlabel = bwlabel(moving);
movind = find(movlabel > 0);
% backim = zeros(size(backim));
...
for ii = 1:length(movind)  %53101:53520
    ...
    posarray = posarray(1:end-1,:);
    x = posarray(:,1); y = posarray(:,2);
    
    % smooth tail
    windowWidth = 3;  % caution: inline params
    polynomialOrder = 2;
...
% 
% movlabel = bwlabel(moving);
% nomovlabel = bwlabel(1-moving);
% 
% for tti = 1:max(movlabel)+1
%     if tti == 1
%         st = 0;
%     else
%         st = alltheta(find(movlabel==(tti-1), 1, 'last'));
%     end
%     if tti > max(movlabel)
%         ed = 0;
%     else
%         ed = alltheta(find(movlabel==tti, 1));
%     end
%     alltheta(nomovlabel==tti) = linspace(st, ed, sum(nomovlabel==tti));
% end
...
save([savepath '/alltheta.mat'], 'alltheta');
