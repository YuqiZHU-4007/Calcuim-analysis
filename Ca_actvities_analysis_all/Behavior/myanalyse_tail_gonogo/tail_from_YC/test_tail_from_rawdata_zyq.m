...
%% load images
imgpath = uigetdir('H:\1.close loop\1.Data from 126\');
% [~, name, ext] = fileparts(imgname);
savepath = checkpath([imgpath '_find_tail/']);
...
tic;
[stdwin, stdwintail] = moving_std_readseq(imgpath, winsize, tailind);
...
for fi = movind(:)'
    ...
    frame=imread(fullfile(imgpath,num2str(fi,'%07d')),'tif');
    frame = double(frame);
