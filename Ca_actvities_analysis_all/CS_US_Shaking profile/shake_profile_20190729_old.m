...
for batchi=1:size(actbatch,1)
    ...
    frame_ind_end=frame.per_cycle;
    nCells_total = size(activities_preCS_dfdf_aftcorrect,2);topN = round(prct_const/100 * nCells_total);%para
    ...
    [stregressors,~,~,~] = GetMotorRegressor(stim,fs_num);
    ...
    for ii=1:7
        ...
        x0 = stimcorr(IX(topN));thr1=max(reg_thres,x0);
        if reg_thres>0
            ind_stim_output = (find(stimcorr>thr1))';
        else
            ind_stim_output = (find(stimcorr<thr1))';
        end
        if x0>reg_thres
        ...
        stAvrcorr = corr(stim_tAvr',M'); stAvrcorr_all(ii,:)=stAvrcorr;
        [~,IX] = sort(stAvrcorr,'descend');
        x0 = stAvrcorr(IX(topN));
        thr2=max(x0,reg_thres2);
        IX_passX = find(stAvrcorr>=thr2);
    ...
    for ii=1:7
        ...
        [~,IX] = sort(stAvrcorr,'descend');
        x0 = stAvrcorr(IX(topN));thr=max(x0,reg_thres2);
