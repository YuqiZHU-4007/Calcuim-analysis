...
function [rawacti,area]=calculate_integtate_dfdf_main_control(activities,cut_move,trial,frame,fs,ind_cut_trial)
%计算曲线下积分
...
area_win_acq=frame.cs_start:frame.cs_end-1;%frame.us_start
area_win_hab_tst=frame.cs_start:frame.cs_end-1;
type_cal_area='event';%type={'lowest','lowest_all','polyfit','first','mean','sum','event'};
ref_win=ceil(0/fs.ca+1):frame.us_start-1;%取取时间段的baseline作为参考判断event
