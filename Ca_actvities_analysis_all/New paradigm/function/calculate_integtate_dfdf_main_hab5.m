...
%计算hab后5个trial以及test前5个trial的CS-US的曲线面积
function [rawacti,area]=calculate_integtate_dfdf_main_hab5(activities,cut_move,trial,frame,fs,ind_cut_trial)
%计算曲线下积分
...
area_win_hab_tst=frame.cs_start:frame.us_start-1;
...
if iscell(activities)
    ...
    for ii=1:max(size(activities))
        ...
        for jj=1:size(a,2)
            ...
            rawacti.hab{ii,1}(:,jj)=mean(aa(trial.hab(3)-5+1:trial.hab(3),:),1);
            rawacti.tst{ii,1}(:,jj)=mean(aa(trial.test(2):trial.test(2)+5-1,:),1);
            ...
            for zz=1:trial.acq_block_num
                ...
                if ~isempty(mean_trial)
                     ...
                     rawacti.acq_mean_blocks{ii,jj}(:,zz)=zeros(size(mean(aa(mean_trial,:),1)));
                end
            end
        end
    end
