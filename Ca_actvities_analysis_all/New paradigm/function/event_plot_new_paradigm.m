function area_mean=event_plot_new_paradigm(locpath,activities_preCS_dfdf_aftcorrect,activities_event_preCS,area,cut_move,ind_cut_trial,outputpath,hvisible)
global fs
global frame
global frameb
global trial
...
%     end
    ...
    set(gcf,'Units','normalized', 'position',[0.05,0.05,0.9,0.6]) ;
    area_mean_across_rois=plot_meantrace_acq(y,fs,trial,frame,cut_move,ind_cut_trial);
    saveas(hh1,[ outpath '\'  'meantrace_acq' '.tif']);
    savefig(hh1,[ outpath '\'  'meantrace_acq']);
    close(hh1);
    %mean_hab_test
    hh2=figure('visible',hvisible);
    [~,~]=plot_mean_hab_test(y,fs,trial,frame,frameb,startpoint,cut_move,ind_cut_trial);
