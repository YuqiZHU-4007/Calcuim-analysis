...
function base=getbaseline_cut3sd_strict_for_new_paradigm(aa,indcut,trial)
bb=aa(:,indcut);
...
sd_test(find(sd_test>mean(sd_test)+1*std(sd_test)))=[];%只取波动较小的（认为没有反应）trial做参考,可以适当取小，20190328
%bb_useforbaseline=b;
kk=1;
for ii=1:size(aa,1)%trial.hab+1:trial.hab+trial.acq
    percent=1;
    if sd(ii)>=mean(sd_test)+2*std(sd_test) && percent<0.5
        warning('baseline is not well');
    end
    while sd(ii)>=mean(sd_test)+2*std(sd_test) && percent>=0.5
    ...
    base.baseline_cut_percent_results(ii,1)=percent;
end
%%%
