function base=getbaseline_cut3sd_strict_for_new_paradigm_partitioned(aa,indcut,trial)
%improved  from getbaseline_cut3sd_strict_for_new_paradigm.m in 20190327
%为了更好的算integrate dfdf, 不为负值
%未改完
bb=aa(:,indcut);
...
sd_test(find(sd_test>mean(sd_test)+1*std(sd_test)))=[];%只取波动较小的（认为没有反应）trial做参考
%bb_useforbaseline=b;
kk=1;
