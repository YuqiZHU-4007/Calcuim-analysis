function event=getevent_Trianglefitting_20180929(a,baseline,maxthres,rasing_width,decling_width,frame,base)
...
ii=3;jj=30; a=activities_preCS_dfdf_aftcorrect{ii}(:,jj);
    ...
    if ind(kk)>event.end_ind(end) && a(ind(kk))>=a(max(ind(kk)-1,1))
        for zz=width_max(kk):-1:1
            if sum(a(ind(kk):ind(kk)+zz-1)>=baseline(ind(kk):ind(kk)+zz-1))>=zz-1
                ...
                if sum(a(max_ind+ind(kk)-1:max_ind+ind(kk)-1+1)>=maxthres(ind(kk)))==2
                    if max(sm_a(ind(kk):ind(kk)+zz-1))>=maxthres(ind(kk)) && rasing_dur>=rasing_width(1) && rasing_dur<=rasing_width(2)...
                        ...
                        if rasing_dur==1 %&& sm_a(ind(kk))>=1.05*maxthres(ind(kk))
