function event=getevent_Trianglefitting_strict(a,baseline,maxthres,rasing_width,decling_width,frame,trial,base)
...
xx=1;event.end_ind=[];event.ind=[];event.end_ind(xx)=0;event.rep_ind=zeros(size(a));
...
%     end
    ...
    event.end_ind(find(ceil(event.ind/frame.per_cycle)>trial.hab & ceil(event.ind/frame.per_cycle)<=trial.hab+trial.acq & mod(event.ind,frame.per_cycle)<=8))=[];
    event.dur(find(ceil(event.ind/frame.per_cycle)>trial.hab & ceil(event.ind/frame.per_cycle)<=trial.hab+trial.acq & mod(event.ind,frame.per_cycle)<=8))=[];
    event.rasing_dur(find(ceil(event.ind/frame.per_cycle)>trial.hab & ceil(event.ind/frame.per_cycle)<=trial.hab+trial.acq & mod(event.ind,frame.per_cycle)<=8))=[];
    event.decline_dur(find(ceil(event.ind/frame.per_cycle)>trial.hab & ceil(event.ind/frame.per_cycle)<=trial.hab+trial.acq & mod(event.ind,frame.per_cycle)<=8))=[];
    event.max_ind(find(ceil(event.ind/frame.per_cycle)>trial.hab & ceil(event.ind/frame.per_cycle)<=trial.hab+trial.acq & mod(event.ind,frame.per_cycle)<=8))=[];
    event.rep_ind(find(ceil(event.ind/frame.per_cycle)>trial.hab & ceil(event.ind/frame.per_cycle)<=trial.hab+trial.acq & mod(event.ind,frame.per_cycle)<=8))=0;
    event.ind(find(ceil(event.ind/frame.per_cycle)>trial.hab & ceil(event.ind/frame.per_cycle)<=trial.hab+trial.acq & mod(event.ind,frame.per_cycle)<=8))=[];
end
