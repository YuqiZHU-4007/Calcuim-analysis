...
for batchi=1:4
    ...
    for fishi=1:length(path)
        ...
        p=path{fishi};
        load(fullfile(p,'CR_ind_summary.mat'), 'CRon_activation_ind','CRon_inhibition_ind');
        load(fullfile(p,'CR_ind_summary.mat'),'ind_all_CSUS_RESPONSIVE','CSon_activation_response','US_activation_response');
...
for batchi=1:4  
   ...
   for fishi=1:length(path)
       ...
       filename=Mahalanobis_mean_region_CSCS;
       for typei=1:length(type)
