...
for batchi=[1,4]
    ...
    for fishi=1:length(Path{batchi})
        ...
        if slidwin==1
            slidwin=6;trial_ind_session=nan(slidwin,(trial.acq_block_num*trial.acq_block_trial-slidwin+1)+2);
            ...
            trial_ind_session=nan(trial.hab(1),trial.acq_block_num+2);
            for sessioni=1:trial.acq_block_num+2
                switch sessioni
                    case 1
                        trial_ind_session(:,sessioni)=trial.hab(2):trial.hab(3);
                    case mat2cell([2:trial.acq_block_num+1],1,ones(1,trial.acq_block_num));
                        trial_ind_session(1:trial.acq_block_trial,sessioni)=trial.hab(3)+(sessioni-2)*trial.acq_block_trial+1 :trial.hab(3)+(sessioni-1)*trial.acq_block_trial;
    ...
    figure, imagesc(p4,[0 1]);hold on;
end
%% 缩放activity
for batchi=[1:4]
    for fishi=1:length(Path{batchi})
        if batchi==1 |batchi==4
            ind=align_trial(batchi,fishi);
        else
            ind=nan;
        end
        path=string(Path{batchi}(fishi))
        ...
        go_trial_hab=intersect(go_trial_cs,[trial.hab(2):trial.hab(3)])';
        go_trial_tst=intersect(go_trial_cs,[trial.test(2):trial.test(3)])';
        nogo_trial_hab=intersect(nogo_trial_cs,[trial.hab(2):trial.hab(3)])';
        nogo_trial_tst=intersect(nogo_trial_cs,[trial.test(2):trial.test(3)])';
        ...
        for ii=1:fix_trial
            activities_dfdf_align(:,ii,:)=squeeze(mean(A_r(:,align_win(:,ii),:) ,2,'omitnan'));
        ...
        a=mean(A_r(:,trial.hab(2):trial.hab(2)+3,:) ,2,'omitnan');
        b=mean(A_r(:,trial.test(2):trial.test(3),:) ,2,'omitnan');load([savepath '\Path']);
for batchi=1:4
    for fishi=1:length(Path{batchi})
        path=string(Path{batchi}(fishi))
...
%         title 'Cluster Assignments and Centroids';hold off;
       ...
       path=string(Path{batchi}(fishi))
        ...
        a=mean(A_r(:,go_trial_hab,:) ,2,'omitnan');
        b=mean(A_r(:,go_trial_tst,:) ,2,'omitnan');
        activities_dfdf_align_go=cat(2,a,activities_dfdf_align_go,b);
        a=mean(A_r(:,nogo_trial_hab,:) ,2,'omitnan');
        b=mean(A_r(:,nogo_trial_tst,:) ,2,'omitnan');
