...
%% 选取Cb
if exist(fullfile(path,'mask_Cb.mat'))~=0
    load(fullfile(path,'mask_Cb.mat'));
else
    ...
    save(fullfile(path,'mask_Cb.mat'),'mask');
end
...
nexttile(t,[3,1]);imagesc(a',[0.01 0.04]);colormap('hot');hold on;
...
n=2;
Cor=corr(a);%figure,imagesc(Cor,[0 0.3]);colorbar;%axis equalcolormap('hsp');
[idx_functional_CR,~,~,~] = kmeans(Cor',n,'Distance','correlation','Replicates',10);
[idxAs_CR,idxAa_ind_CR]=sort(idx_functional_CR);
nexttile(t,[1,1]);imagesc(Cor(idxAa_ind_CR,idxAa_ind_CR),[0 0.5]);colormap('jet');colorbar;%axis equal
...
save(fullfile(path,'Cb_summary.mat'),'AUC','AUC_go','AUC_nogo','AUC_us',...
...
save(fullfile(path,'mask_Cb.mat'),'mask','Cb_ind_new_emerged','Spatial_density','AUC_go','AUC_nogo','AUC_us',...
'idxAa_ind_CR','idxAs_CR','AUC_go_clust','AUC_nogo_clust','AUC_us_clust','AUC');
