...
if 1%~(exist([fullfile(p,'singletrial'),'\CR_ind_summary_singletrial.mat'],'file')==2)
    ...
    if ~[exist(fullfile(p,'activities_aft_process.mat'),'file') && exist(fullfile(p,'para.mat'),'file') && exist(fullfile(p,'env.mat'),'file')]
        ...
        Fraction_in_region_type_emerged=cell(length(labels_typei),trial.total,2,2,1);Index_in_region_emerged=cell(length(labels_typei),trial.total,trial.total,2,2,1);
        Loc_in_region_cell_emerged=cell(length(labels_typei),trial.total,trial.total,2,2,1);Num_in_region_emerged=cell(length(labels_typei),trial.total,2,2,3,1);
        Brain_region_id_emerged=cell(length(labels_typei),trial.total,trial.total,2,2,1);Label_region_emerged=cell(length(labels_typei),trial.total,2,2,1);
        ind_all_emergeCSUS_inthissession=cell(length(labels_typei),trial.total,2,3);
        Fraction_in_region_type_emerged_inthissession=cell(length(labels_typei),trial.total,2,2,3);Index_in_region_emerged_inthissession=cell(length(labels_typei),trial.total,trial.total,2,3);
        Loc_in_region_cell_emerged_inthissession=cell(length(labels_typei),trial.total,trial.total,2,2,3);Num_in_region_emerged_inthissession=cell(length(labels_typei),trial.total,2,2,3,3);
        Brain_region_id_emerged_inthissession=cell(length(labels_typei),trial.total,trial.total,2,2,3);Label_region_emerged_inthissession=cell(length(labels_typei),trial.total,2,2,3);
        ...
        for iscutmov=1
            ...
            if 0
                for jj=1:length(labels_typei)
                    gIX=[];supervoxel_in_fish=[];
                    for ii=1:trial.total
                        ind=ind_all_CSUS_RESPONSIVE{jj,ii,iscutmov};
                        if ~isempty(ind)
                            gIX=cat(1,gIX,ones(1,length(ind))'*ii);supervoxel_in_fish=cat(1,supervoxel_in_fish,supervolxeli(ind,1:3));
                        end
                    end
                    for fraction_type=1
                        ...
                        clrmap = GetColormap('hsv_new',max(gIX));
                        [ind_in_region_in_clust,fraction_in_region_in_clust,loc_in_region_cell,num_in_region_in_clust,brain_region_id,Label]=get_region_fraction_temp_preprocess(gIX,supervoxel_in_fish,reg_mask,reg_name,reg_loc,temp_vol,temp_env,clrmap,nn);
                        Fraction_in_region_type{jj,iscutmov,fraction_type}=fraction_in_region_in_clust(:,1:trial.total);
                        for ii=1:trial.total
                            Index_in_region{jj,ii,iscutmov,fraction_type}=ind_in_region_in_clust{:,ii};
                            Loc_in_region_cell{jj,ii,iscutmov,fraction_type}=loc_in_region_cell;
                            Num_in_region{jj,iscutmov,fraction_type,1}(:,ii)= num_in_region_in_clust{1}(:,ii);
                            Num_in_region{jj,iscutmov,fraction_type,2}(:,ii)= num_in_region_in_clust{2}(:,ii);
                            Num_in_region{jj,iscutmov,fraction_type,3}(:,ii)= num_in_region_in_clust{3}(:,ii);
                            Brain_region_id{jj,ii,iscutmov,fraction_type}=brain_region_id{ii}(:,1);
                ...
                for jj=1:length(labels_typei);%[1:18,31:34]
                    for typei=1:size(ind_all_emergeCSUS_inthissession,4)
                        gIX=[];supervoxel_in_fish=[];
                        for ii=1:trial.total
                            ind=ind_all_emergeCSUS_inthissession{jj,ii,iscutmov,typei};
                            if ~isempty(ind)
                                gIX=cat(1,gIX,ones(1,length(ind))'*ii);supervoxel_in_fish=cat(1,supervoxel_in_fish,supervolxeli(ind,1:3));
                            end
                        end
                        for fraction_type=1
                            ...
                            clrmap = GetColormap('hsv_new',max(gIX));
                            [ind_in_region_in_clust,fraction_in_region_in_clust,loc_in_region_cell,num_in_region_in_clust,brain_region_id,Label]=get_region_fraction_temp_preprocess(gIX,supervoxel_in_fish,reg_mask,reg_name,reg_loc,temp_vol,temp_env,clrmap,nn);
                            Fraction_in_region_type_emerged_inthissession{jj,zz,iscutmov,fraction_type,typei}=fraction_in_region_in_clust(:,1:trial.total);
                            for ii=1:trial.total
                                Index_in_region_emerged_inthissession{jj,zz,ii,iscutmov,fraction_type,typei}=ind_in_region_in_clust{:,ii};
                                Loc_in_region_cell_emerged_inthissession{jj,zz,ii,iscutmov,fraction_type,typei}=loc_in_region_cell;
                                Num_in_region_emerged_inthissession{jj,zz,iscutmov,fraction_type,1,typei}(:,ii)= num_in_region_in_clust{1}(:,ii);
                                Num_in_region_emerged_inthissession{jj,zz,iscutmov,fraction_type,2,typei}(:,ii)= num_in_region_in_clust{2}(:,ii);
                                Num_in_region_emerged_inthissession{jj,zz,iscutmov,fraction_type,3,typei}(:,ii)= num_in_region_in_clust{3}(:,ii);
                                Brain_region_id_emerged_inthissession{jj,zz,ii,iscutmov,fraction_type,typei}=brain_region_id{ii}(:,1);
                                Label_region_emerged_inthissession{jj,zz,iscutmov,fraction_type,typei}(:,ii)=Label;
                            end
                        end
                    end
                end
                for jj=1:length(labels_typei);%[1:18,31:34]
                    for  typei=1:size(ind_all_emergeCSUS,5)
                        ...
                        for fraction_type=1
                            ...
                            clrmap = GetColormap('hsv_new',max(gIX));
                            [ind_in_region_in_clust,fraction_in_region_in_clust,loc_in_region_cell,num_in_region_in_clust,brain_region_id,Label]=get_region_fraction_temp_preprocess(gIX,supervoxel_in_fish,reg_mask,reg_name,reg_loc,temp_vol,temp_env,clrmap,nn);
                            Fraction_in_region_type_emerged{jj,zz,iscutmov,fraction_type,typei}=fraction_in_region_in_clust(:,1:trial.total);
                            for ii=1:trial.total
                                Index_in_region_emerged{jj,zz,ii,iscutmov,fraction_type,typei}=ind_in_region_in_clust{:,ii};
                                Loc_in_region_cell_emerged{jj,zz,ii,iscutmov,fraction_type,typei}=loc_in_region_cell;
                                Num_in_region_emerged{jj,zz,iscutmov,fraction_type,1,typei}(:,ii)= num_in_region_in_clust{1}(:,ii);
                                Num_in_region_emerged{jj,zz,iscutmov,fraction_type,2,typei}(:,ii)= num_in_region_in_clust{2}(:,ii);
                                Num_in_region_emerged{jj,zz,iscutmov,fraction_type,3,typei}(:,ii)= num_in_region_in_clust{3}(:,ii);
                                Brain_region_id_emerged{jj,zz,ii,iscutmov,fraction_type,typei}=brain_region_id{ii}(:,1);
                                Label_region_emerged{jj,zz,iscutmov,fraction_type,typei}(:,ii)=Label;
                            end
                        end
                    end
                end
