...
savepath=uigetdir('E:\A_Data_lightsheet\Data_vmat\Summary\Calcium summary\integrate dfdf\all_learner_huc_cutmove_20190619\huc_learner_cutmove_20190617\path\','savepath');
% for kk=1:6
...
% end
save([savepath '\path'],'actbatch','envbatch','behavbatch','parabatch','-v7.3');
...
for kk=1:size(actbatch,1)
    ...
    ind_cut_trial_CS=re_startpoint(find(re_startpoint(:,2)>=(frameb.cs_start) & re_startpoint(:,2)<(frameb.cs_end)),1);%%%%%%%%%%%%%%%%
    ind_cut_trial_CS=unique(ind_cut_trial_CS);
    ind_cut_trial=ind_cut_trial_CS;%union(ind_cut_trial_preCS,ind_cut_trial_CS);
    ...
    for ii=1:i_ind
        switch ii
            ...
            case 2
                ...
                aa=[];
                for jj=trial_ind
                    a=act(:,(jj-1)*frame.per_cycle+frame.cs_start:(jj-1)*frame.per_cycle+frame.cs_end-1)';
                    aa=[aa;a];
                end
...
ind=ind([1:10:length(ind)]);
[gIX,~,numK,~]=find_best_k_in_range(M_0(ind,:),2);
cIX=ind;cIX_reg =(1:size(M_0,1))';
if isempty(numK)
    numK=3;
end
masterthres=0.6;
...
for iii=1:2
    ...
    %3
    h=figure;B=area_all.CS_acq_block;%B=[area.CS_hab_tst(1,:); area.CS_acq_block ;area.CS_hab_tst(2,:)];
    a=[-0.04 0.1];plot_test_1([1:5],B,cIX_iii,gIX_iii,clrmap_iii,a,frame,fs.ca);saveas(h,[outputpath '\f3'],'fig');
    %4
    [h,ratio]=plot_test_2(act_all_acq_block_mean,cIX_iii,gIX_iii,frame,clrmap_iii,1,[-0.01 0.06]);saveas(h,[outputpath '\f41'],'fig');
    [h,ratio]=plot_test_2(act_all_acq_block_mean,cIX_iii,gIX_iii,frame,clrmap_iii,2,[-0.01 0.06]);saveas(h,[outputpath '\f42'],'fig');
    %5
    [h,~]=plot_test_2(act_all_hab_mean,cIX_iii,gIX_iii,frame,clrmap_iii,2,[-0.01 0.06]);saveas(h,[outputpath '\f5'],'fig');
    %6
    [h,~]=plot_test_2(act_all_tst_mean,cIX_iii,gIX_iii,frame,clrmap_iii,2,[-0.01 0.06]);saveas(h,[outputpath '\f6'],'fig');
    %7
    [ratio_fish]=plot_test_3(act_all_acq,cIX_iii,gIX_iii,clrmap_iii,index_all,envbatch,env_all,outputpath,fs,stimCS,stimUS);
end
...
num_clust=[6,7];cIX_iii=cIX;gIX_iii=gIX;clrmap_iii=clrmap;a=[];
