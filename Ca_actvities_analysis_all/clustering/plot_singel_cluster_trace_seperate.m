...
h_base=nan(1,max(unique(cIX_iii)))';p_base=nan(1,max(unique(cIX_iii)))';h_base_adj=zeros(1,max(unique(cIX_iii)))';
...
[FDR_base,q_base] = mafdr(p_base);h_base_adj(find(q_base<0.05))=1;
...
rank_ind=find(h_base_adj==1 & h_tst_adj==0);rank_ind=find(h_tst_adj==1);rank_ind=find(h_base_adj==1 & h_tst_adj==1);%i_ind=i_ind(500);
