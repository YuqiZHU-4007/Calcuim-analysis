function D = distance_mahal(X, Y, W)
%Y样本数据，X观测数据
%MAHALDIST Mahalanobis distance.
%
%   D = distance_mahal(X, Y, W) computes a distance matrix D where the element
...
%
%   distance_mahal(X, [], W) is the same as distance_mahal(X, X, W), but the former is
%   computed more efficiently.
%
%   distance_mahal([], Y, W) is the same as distance_mahal(Y, Y, W), but the former is
%   computed more efficiently.
%
%   D = distance_mahal(X, Y) is the same as distance_mahal(X, MEAN(Y,1), INV(COV(Y))),
...
%
%   distance_mahal(X) is the same as distance_mahal(X, X), but the former is computed
%   more efficiently.
%
%   NOTE: The Mahalanobis distance is sometimes defined as the square root of
%   the values returned by distance_mahal.
%
...
%
%   When X and Y are real, then MAHAL(X, Y) is the same as distance_mahal(X, Y)
%   and PDIST(X, 'mahal') returns the lower triangular part of
%   SQRT(distance_mahal(X, [], INV(COV(X)))).  Both MAHAL and PDIST are in the
%   Statistics Toolbox.
%
