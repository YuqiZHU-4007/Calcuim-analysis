...
for ii=1:size(activities,1)
    ...
    for jj=1:size(a,2) 
        aa=a(:,jj); sort_aa=sort(aa); sort_aa= sort_aa(1:length( sort_aa)*0.975);
...
for ii=1:size(activities,1)
    ...
    for jj=1:size(a,2) 
        ...
        event=getevent_Trianglefitting(a(:,jj),baseline,maxthres,rasing_width,decling_width,frame,trial,base);
...
for ii=1:size(activities,1)
    for jj=1:size(activities{ii},2)
       ...
       plot([1:frame.per_cycle*trial.total]*0.4,a);hold on;xlim([1 trial.hab*frame.per_cycle]*0.4);hold on;
        ...
        switch st
            case 1
                saveas(h1,[outpath '\raw trace\' num2str(ii) 'layer-' num2str(jj) '.tif']);
                saveas(h2,[outpath '\raw trace\' num2str(ii) 'layer-' num2str(jj) '.tif']);
               % disp('1')
            case 2
                saveas(h1,[checkpath([outpath '\hotmap\' num2str(ii) 'layer\' num2str(jj) '\']) num2str(ii) 'layer-' num2str(jj) '.tif']);
