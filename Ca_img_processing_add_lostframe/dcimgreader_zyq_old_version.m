classdef dcimgreader < handle %Image4DReader
    ...
    methods
        function obj = dcimgreader(imgpath, T, t0,process)
        ...
        function frame = readframe(obj, ti, zi)%%%
            ...
            if isempty(obj.T_lessframe,2) && isempty(obj.T_moreframe,2)%%%存不存在多帧少帧情况
                fi = obj.t0 + obj.T * (ti-1) + zi - 1;
                frame = dcimgread(obj.fhandle, fi);
            else %存在多帧少帧
                if (length(indl)~=0 && length(find(obj.z_lessframe(indl,:)==zi))~=0) %%%ti的zi不存在,在少帧情况里
