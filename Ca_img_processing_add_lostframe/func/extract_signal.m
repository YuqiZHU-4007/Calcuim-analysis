function [duration_extract,activities,backgroundy,backPSTH ]=extract_signal(opt,env)
...
if opt.extract_signal_multicore  % extract with spmd
    ...
    mask = imdilate(env.mask, strel('disk', env.width/16+1));%%≈Ú’Õmask
    figure; imshow(mask)
    ...
    if opt.isregistration%%already registered above, load registered .tif files
        ...
        spmd(poolsize)
            ...
            imio = dcimgreader_zyq(opt.imgpath, opt.T, [],opt.process);  %, opt.t0); % auto estimate t0
