function registered_io = pipeline_registration_ori_zyq(env, opt, prct)
...
Compression='lzw'; %写出tif时压缩，20190606，zyq

depth = env.depth;%实际拍的brain的层数
nvol = env.nvol;
...
spmd(poolsize)
    % zi = 17;
    imio = dcimgreader_zyq(opt.imgpath, opt.T, opt.t0,opt.process);
    
    
    for zi = labindex:poolsize:env.depth; % %labindex:poolsize:env.depth
        [labindex zi]
        ...
        % save for ti=1
        imwrite(uint16(reg_frame), fullfile([zpath '/' num2str(t1, '%04d') '.tif']),'Compression',Compression);
        a=imfinfo(fullfile([zpath '/' num2str(t1, '%04d') '.tif']));
        while exist(fullfile([zpath '/' num2str(t1, '%04d') '.tif']),'file')==0
            imwrite(uint16(reg_frame), fullfile([zpath '/' num2str(t1, '%04d') '.tif']),'Compression',Compression);
            a=imfinfo(fullfile([zpath '/' num2str(t1, '%04d') '.tif']));
        end
        while a.FileSize<40000 %|| a.FileSize>2900000
            imwrite(uint16(reg_frame), fullfile([zpath '/' num2str(t1, '%04d') '.tif']),'Compression',Compression);
            a=imfinfo(fullfile([zpath '/' num2str(t1, '%04d') '.tif']));
            warning(['write error in ' fullfile([zpath '/' num2str(t1, '%04d') '.tif'])]);
        end
        ...
        for ti = subsamplet(2:end)  %2:4  %nvol
            ...
            imwrite(uint16(reg_frame), fullfile([zpath '/' num2str(ti,'%04d') '.tif']),'Compression',Compression);
            a=imfinfo(fullfile([zpath '/' num2str(ti,'%04d') '.tif']) );
            while exist(fullfile([zpath '/' num2str(ti,'%04d') '.tif']) ,'file')==0
                imwrite(uint16(reg_frame), fullfile([zpath '/' num2str(ti,'%04d') '.tif']),'Compression',Compression);
                a=imfinfo(fullfile([zpath '/' num2str(ti,'%04d') '.tif']) );
            end
            while a.FileSize<40000 %|| a.FileSize>2900000
                imwrite(uint16(reg_frame),fullfile([zpath '/' num2str(ti,'%04d') '.tif']) ,'Compression',Compression);
                a=imfinfo(fullfile([zpath '/' num2str(ti,'%04d') '.tif']));
                warning(['write error in ' fullfile([zpath '/' num2str(ti,'%04d') '.tif'])]);
            end
