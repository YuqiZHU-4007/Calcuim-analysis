function registered_io = pipeline_registration(env, opt, prct)
...
%spmd(poolsize)
%zi = 17;
%imio = dcimgreader(opt.imgpath, opt.T, opt.t0);
imio = env.imio;
...
[reg_frame, reg_params] = frame_registration_prepared_reference(pre_refr, prev_frame, warpopt);
isreg(t1) = true;
...
dlmwrite([reginfopath  '/' num2str(t1, '%04d') '.txt'], ...
        [isreg(t1) isreg(t1); cor_to_prev(t1) dif_to_prev(t1); reg_params]);
    
for ti = t1:nvol  %subsamplet(2:end)  %2:4  %nvol
