classdef dcimgreader_zyq < handle %Image4DReader
    ...
    methods
        function obj = dcimgreader_zyq(imgpath, T, t0,process)
            ...
            obj.nframes = obj.fhandle.nframes-getnonXnum(obj.z_moreframe,0)+getnonXnum(obj.z_lessframe,0);
        ...
        function frame = readframe(obj, ti, zi)%%%可改
            mf=0;mz=0;lf=0;lz=0;
            indm=find(obj.T_moreframe==ti);indl=find(obj.T_lessframe==ti);
            if isempty(obj.T_lessframe) && isempty(obj.T_moreframe)%%%存不存在多帧少帧情况
                fi = obj.t0 + obj.T * (ti-1) + zi - 1;
                frame = dcimgread(obj.fhandle, fi);
            else %存在多帧少帧
                if (~isempty(indl) && ~isempty(find(obj.z_lessframe(indl,:)==zi))) %%%ti的zi不存在,在少帧情况里
