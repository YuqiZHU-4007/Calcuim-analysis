...
savepath='X:\calcium data 20230224\';
load([savepath '\Path']);
load([savepath '\align_trial.mat'],'slidwin','trial_ind_session');
p3={};
for batchi=1:4
    p3=[];
    for fishi=1:length(Path{batchi})
        ...
        load(fullfile(path,'\para.mat'));
        load(fullfile(path, '/activities_dfdf_align.mat'),'align_win','align_win_go','align_win_nogo');
        [p_value,bef_cond,dur_cond,aft_cond]=tail_movement_by_trial_fin_cor(y_3sd,re_startpoint_sd,[],[],[],1);
        x=cat(1,bef_cond.CS_shake,dur_cond.CS192_shake,aft_cond.CS_shake);
        for sessioni=1:size(align_win,2)
            a=align_win(:,sessioni);a(isnan(a))=[];
            p3{batchi}(sessioni,fishi)=sum(x(a))./length(a);
        end 
    end
end
