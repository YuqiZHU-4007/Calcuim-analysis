function US_mapping(Path)
set(0,'defaultfigurecolor','w')
addpath(genpath('X:\calcium data 20230224\script\FishExplorer'));
if isempty(Path)
    p=uigetdir('H:\1.Test US\2.Tail free——Data from 117\')
else
    p=Path
end
if ~(exist([p,'CR_ind_summary.mat'],'file')==2)
    if ~[exist(fullfile(p,'activities_aft_process.mat'),'file') && exist(fullfile(p,'para.mat'),'file') && exist(fullfile(p,'env.mat'),'file')]
        ...
        trial_test=[trial.test(2):trial.test(3)];[38 39 43 44 45 46];%trial.test(2):trial.test(2)+5;
        ...
        time_ca=0:fs.ca:(length(A(:,1))-1)*fs.ca;
        colorCS=[0.9 0.5 0.5];
        ...
        for ii=1:2
            ...
            a=re_startpoint(find(re_startpoint(:,1)>=ind(1) &  re_startpoint(:,1)<=ind(end)...
                & re_startpoint(:,2)>=frameb.cs_start-6*fs.behavior & re_startpoint(:,2)<frameb.cs_start-1*fs.behavior),1);
            ...
            if ii==1
                baseline_up=m+ 2*sd; baseline_down=m+ 1*sd;baseline_UR=m+2*sd;
            end
            aa=A_r(:,ind,:);
            for iscutmov=1:2 %1:no_cut;2:cut
                b=nan(length(us_win),length(ind),size(A,2));
                for jj=1:length(ind)
                    ind(jj);
                    if iscutmov==2
                        a=re_startpoint(find(re_startpoint(:,1)==ind(jj) ...
                            & re_startpoint(:,2)>=frameb.cs_start-10 & re_startpoint(:,2)<frameb.cs_end),2);
                        if ~isempty(a)
                            b(1:length(frame.cs_start:floor(a/fs.behavior/fs.ca)),jj,:)=aa(frame.cs_start:floor(a/fs.behavior/fs.ca),jj,:);
                        else
                            b(:,jj,:)=aa(us_win,jj,:);
                        end
                    else
        ...
        for ii=1:trial.acq_block_num
            ...
            %         aa=A_r(:,ind,:);
            %         for jj=1:length(ind)
            %             zz=find(re_startpoint(:,1)==ind(jj) & re_startpoint(:,2)<base_win(end)*fs.ca*fs.behavior & re_startpoint(:,2)>base_win(1)*fs.ca*fs.behavior)
            %             if ~isempty(zz)
            %                 for hh=1:length(zz)
            %                     aa(max(round(re_startpoint(zz(hh),2)/fs.behavior/fs.ca)-2,1):round(re_startpoint(zz(hh),2)/fs.behavior/fs.ca)+8,jj,:)=nan;
            %                 end
            %             end
            %         end
            %         aa=squeeze(mean(aa(base_win,:,:),2,'omitnan'));
            %         m=mean(aa,1,'omitnan');sd=std(aa,[],1,'omitnan');
            aa=A_r(:,ind,:);
            for iscutmov=1:2
                b=nan(length(us_win),length(ind),size(A,2));
        ...
        %% US引起尾动
        trial_ind=trial.acq(2):trial.acq(3);
        bb=B_r;bb=bb(:,trial_ind);
        bb(1:frameb.cs_start,:)=0;
        y=reshape(bb,[],1);
        for ii=1
            behavior=zeros(5,length(trial_ind)*frameb.per_cycle);behavior(1,:)=abs(y);
            [regressors,~,~,~] = GetMotorRegressor(behavior,6);
            regressors_motor_down = interp1(0:1/fs.behavior:(length(y)-1)/fs.behavior,regressors(1,1).im,0:fs.ca:(length(trial_ind)*frame.per_cycle-1)*fs.ca);
            M=A_r(:,trial_ind,:);M=reshape(M,frame.per_cycle*length(trial_ind),[]);
            [~,mtRescorr] = MotorSourceCorrelation(M',[],regressors_motor_down);
            %     [~,IX] = sort(mtRescorr,'descend');
            %     x0 = mtRescorr(IX(topN));x0=max(x0,0.55);
            %     ind_motor_output=find(mtRescorr> x0);
            %     motor_output=mean(A(:,ind_motor_output)',1);
            %     [~,mtRescorr] = MotorSourceCorrelation(A',[],motor_output);
            figure,plot(0:1/fs.behavior:(length(y)-1)/fs.behavior,behavior(1,:));hold on;
            plot(0:fs.ca:(length(trial_ind)*frame.per_cycle-1)*fs.ca,regressors_motor_down,'r');legend('raw','regressor')
            thr=mean(mtRescorr)+1*std(mtRescorr);
            figure,hist(mtRescorr);hold on;line([thr,thr],[0 2000],'color','r','linewidth',3);
            USmotor_related_ind=find(mtRescorr>thr);
        end
        %% test期间 CS引起尾动
        trial_ind=trial.test(2):trial.test(2)+5;
        bb=B_r;bb=bb(:,trial_ind);
        %bb(1:frameb.cs_start-1*fs.behavior,:)=0;
        y=reshape(bb,[],1);
        for ii=1
            behavior=zeros(5,length(trial_ind)*frameb.per_cycle);behavior(1,:)=abs(y);
            [regressors,~,~,~] = GetMotorRegressor(behavior,6);
            regressors_motor_down = interp1(0:1/fs.behavior:(length(y)-1)/fs.behavior,regressors(1,1).im,0:fs.ca:(length(trial_ind)*frame.per_cycle-1)*fs.ca);
            M=A_r(:,trial_ind,:);M=reshape(M,frame.per_cycle*length(trial_ind),[]);
            [~,mtRescorr_test] = MotorSourceCorrelation(M',[],regressors_motor_down);
            %     [~,IX] = sort(mtRescorr,'descend');
            %     x0 = mtRescorr(IX(topN));x0=max(x0,0.55);
            %     ind_motor_output=find(mtRescorr> x0);
            %     motor_output=mean(A(:,ind_motor_output)',1);
            ...
            figure,plot(0:1/fs.behavior:(length(y)-1)/fs.behavior,behavior(1,:));hold on;
            plot(0:fs.ca:(length(trial_ind)*frame.per_cycle-1)*fs.ca,regressors_motor_down,'r');legend('raw','regressor')
            thr=mean(mtRescorr_test)+1*std(mtRescorr_test);
            figure,hist(mtRescorr_test);hold on;line([thr,thr],[0 2000],'color','r','linewidth',3);
            CSmotor_related_tst_ind=find(mtRescorr_test>thr);
            figure,
            a=[0 0.05];
            patch1=patch([frame.cs_start frame.cs_end frame.cs_end frame.cs_start],...
                [min(a) min(a) max(a) max(a)],...
                colorCS,'edgecolor',colorCS,'facecolor',colorCS,'edgealpha',0.2,'facealpha',0.25);hold on
            plot(squeeze(mean(A_r(:,trial_test,CSmotor_related_tst_ind),3)))%trial.test(2):trial.test(3)
            figure,
            ind=find(sum(UR_ind(:,:),2)==trial.acq_block_num);
            scatter3(supervoxel_raw(ind,1),supervoxel_raw(ind,2),supervoxel_raw(ind,3),10,'b','filled');hold on
            scatter3(supervoxel_raw(CSmotor_related_tst_ind,1),supervoxel_raw(CSmotor_related_tst_ind,2),supervoxel_raw(CSmotor_related_tst_ind,3),10,'r','filled');
            axis equal;legend('UR ind','CS motor related ind in tst')
        end
        ...
        for ii=1
            ...
            regressors_motor_down = interp1(0:1/fs.behavior:(length(y)-1)/fs.behavior,regressors(1,1).im,0:fs.ca:(length(trial_ind)*frame.per_cycle-1)*fs.ca);
            M=A_r(:,trial_ind,:);M=reshape(M,frame.per_cycle*length(trial_ind),[]);
            [~,mtRescorr_hab] = MotorSourceCorrelation(M',[],regressors_motor_down);
            %     [~,IX] = sort(mtRescorr,'descend');
            %     x0 = mtRescorr(IX(topN));x0=max(x0,0.55);
            %     ind_motor_output=find(mtRescorr> x0);
            %     motor_output=mean(A(:,ind_motor_output)',1);
            ...
            figure,plot(0:1/fs.behavior:(length(y)-1)/fs.behavior,behavior(1,:));hold on;
            plot(0:fs.ca:(length(trial_ind)*frame.per_cycle-1)*fs.ca,regressors_motor_down,'r');legend('raw','regressor')
            thr=mean(mtRescorr_hab)+1*std(mtRescorr_hab);
            figure,hist(mtRescorr_hab);hold on;line([thr,thr],[0 2000],'color','r','linewidth',3);
            CSmotor_related_hab_ind=find(mtRescorr_hab>thr);
        end
        %% 全程 CS引起的尾动
        trial_ind=1:trial.test(2)-1;
        bb=B_r;bb=bb(:,trial_ind);
        y=reshape(bb,[],1);
        for ii=1
            behavior=zeros(5,length(trial_ind)*frameb.per_cycle);behavior(1,:)=abs(y);
            ...
            regressors_motor_down = interp1(0:1/fs.behavior:(length(y)-1)/fs.behavior,regressors(1,1).im,0:fs.ca:(length(trial_ind)*frame.per_cycle-1)*fs.ca);
            M=A_r(:,trial_ind,:);M=reshape(M,frame.per_cycle*length(trial_ind),[]);
            [~,mtRescorr_all] = MotorSourceCorrelation(M',[],regressors_motor_down);
            figure,plot(0:1/fs.behavior:(length(y)-1)/fs.behavior,behavior(1,:));hold on;
            plot(0:fs.ca:(length(trial_ind)*frame.per_cycle-1)*fs.ca,regressors_motor_down,'r');legend('raw','regressor')
            set(gca,'fontsize',16)
            thr=mean(mtRescorr_all)+1*std(mtRescorr_all);
            figure,hist(mtRescorr_all);hold on;line([thr,thr],[0 2000],'color','r','linewidth',3);
            CSmotor_related_all_ind=find(mtRescorr_all>thr);
            figure,
            a=[0 0.05];
            patch1=patch([frame.cs_start frame.cs_end frame.cs_end frame.cs_start],...
                [min(a) min(a) max(a) max(a)],...
                colorCS,'edgecolor',colorCS,'facecolor',colorCS,'edgealpha',0.2,'facealpha',0.25);hold on
            plot(squeeze(mean(A_r(:,trial_test,CSmotor_related_all_ind),3)))%trial.test(2):trial.test(3)
            figure,
            ind=find(sum(UR_ind(:,:),2)==trial.acq_block_num);
            scatter3(supervoxel_raw(ind,1),supervoxel_raw(ind,2),supervoxel_raw(ind,3),10,'b','filled');hold on
            scatter3(supervoxel_raw(CSmotor_related_all_ind,1),supervoxel_raw(CSmotor_related_all_ind,2),supervoxel_raw(CSmotor_related_all_ind,3),10,'r','filled');
            axis equal;legend('UR ind','CS motor related ind in tst')
        end
        %% hab期间 CS引起视觉反应
        trial_ind=trial_hab;trial.hab(2):trial.hab(3);
        ...
        %% test期间 CS引起视觉反应
        trial_ind=trial_test;trial.test(2):trial.test(2)+5;
        bb=reshape(stimCS,frame.per_cycle,trial.total);bb=bb(:,trial_ind);
        y=reshape(bb,[],1);
        for ii=1
            stim=zeros(5,length(trial_ind)*frame.per_cycle);stim(1,:)=y;
        ...
        %% test 期间 CS引起向下的视觉反应
        trial_ind=trial_test;trial.test(2):trial.test(3);
        bb=reshape(stimCS,frame.per_cycle,trial.total);bb=bb(:,trial_ind);
        nCells_total = size(A,2);prct_const = 2 ;topN = round(prct_const/100 * nCells_total);
        y=reshape(bb,[],1);
        for ii=1
        ...
        for iscutmov=1:2;
            ...
            for ii=1:2
                ...
                ylim([-0.008 0.04]);set(gca,'fontsize',16);hold on;
                y=[0.01 0.08];
            ...
            if ranksumtest==1;
                ...
                for ii=1:size(A,2)
                    p_CS_up_bef_aft(ii,iscutmov)=ranksum(mean(squeeze(x(:,:,ii)),1,'omitnan'),mean(squeeze(y(:,:,ii)),1,'omitnan'),'tail','left');
                    p_CS_down_bef_aft(ii,iscutmov)=ranksum(mean(squeeze(x(:,:,ii)),1,'omitnan'),mean(squeeze(y(:,:,ii)),1,'omitnan'),'tail','right');
                    for jj=1:trial.acq_block_num
                        p_CS_up_bef_cond(ii,jj,iscutmov)=ranksum(mean(squeeze(x(:,:,ii)),1,'omitnan'),mean(squeeze(z{jj,iscutmov}(:,:,ii)),1,'omitnan'),'tail','left');
                        p_CS_down_bef_cond(ii,jj,iscutmov)=ranksum(mean(squeeze(x(:,:,ii)),1,'omitnan'),mean(squeeze(z{jj,iscutmov}(:,:,ii)),1,'omitnan'),'tail','right');
                    end
                end
                ...
                for ii=1:size(A,2)
                    for jj=1:trial.acq_block_num
                        p_US_up_bef_cond(ii,jj)=ranksum(mean(squeeze(x(:,:,ii)),1,'omitnan'),mean(squeeze(y{jj}(:,:,ii)),1,'omitnan'),'tail','left');
                        p_US_down_bef_cond(ii,jj)=ranksum(mean(squeeze(x(:,:,ii)),1,'omitnan'),mean(squeeze(y{jj}(:,:,ii)),1,'omitnan'),'tail','right');
                    end
                end
            end
            %plot pie
            ...
            for ii=1:trial.acq_block_num+2
                switch ii
                    case trial.acq_block_num+2
                        CS_up_ind{ii,iscutmov}=find(squeeze(p_CS_up_bef_aft(:,iscutmov))<=0.05 & sum(squeeze(CR_ind_up(:,:,iscutmov)),2)>=1);
                        CS_down_ind{ii,iscutmov}=find(squeeze(p_CS_down_bef_aft(:,iscutmov))<=0.05 & sum(squeeze(CR_ind_up(:,:,iscutmov)),2)>=1);
                        CS_stable_ind{ii,iscutmov}=find((squeeze(p_CS_up_bef_aft(:,iscutmov))>0.05 & squeeze(p_CS_down_bef_aft(:,iscutmov))>0.05) & sum(squeeze(CR_ind_up(:,:,iscutmov)),2)>=1);
                    ...
                    case 1
                        ...
                        US_stable_ind{ii,iscutmov}=[];
                    otherwise
                        CS_up_ind{ii,iscutmov}=find(squeeze(p_CS_up_bef_cond(:,ii-1,iscutmov))<=0.05 & squeeze(CR_ind_up_acq(:,ii-1,iscutmov)+ CR_ind_up(:,1,iscutmov))>=1);
                        CS_down_ind{ii,iscutmov}=find(squeeze(p_CS_down_bef_cond(:,ii-1,iscutmov))<=0.05 &  squeeze(CR_ind_up_acq(:,ii-1,iscutmov)+ CR_ind_up(:,1,iscutmov))>=1);
                        CS_stable_ind{ii,iscutmov}=find((squeeze(p_CS_up_bef_cond(:,ii-1,iscutmov))>0.05 & squeeze(p_CS_down_bef_cond(:,ii-1,iscutmov))>0.05) & squeeze(CR_ind_up_acq(:,ii-1,iscutmov)+ CR_ind_up(:,1,iscutmov))>=1);
                        US_up_ind{ii,iscutmov}=find(squeeze(p_US_up_bef_cond(:,ii-1))<=0.05 & squeeze(UR_ind(:,ii-1)+UR_ind(:,1))>=1);
                        US_down_ind{ii,iscutmov}=find(squeeze(p_US_down_bef_cond(:,ii-1))<=0.05 & squeeze(UR_ind(:,ii-1)+UR_ind(:,1))>=1);
                        US_stable_ind{ii,iscutmov}=find((squeeze(p_US_up_bef_cond(:,ii-1))>0.05 & squeeze(p_US_down_bef_cond(:,ii-1))>0.05) & squeeze(UR_ind(:,ii-1)+UR_ind(:,1))>=1);
                end
    ...
    %% SAVE
    save(fullfile(p,'CR_ind_summary.mat'),'A_r','trial_hab','trial_test',...
