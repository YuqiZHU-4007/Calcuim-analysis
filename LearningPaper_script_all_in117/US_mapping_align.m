...
if 1%~(exist([p,'CR_ind_summary_align.mat'],'file')==2)
    if ~[exist(fullfile(p,'activities_dfdf_align.mat'),'file') && exist(fullfile(p,'para.mat'),'file') && exist(fullfile(p,'env.mat'),'file')]
        ...
        A_r=zscore(A_r,0,1);A_r_n=zscore(A_r,0,1);
        %A_r=reshape(A,size(A_r,1),size(A_r,2),[]);
        res=[0.66,0.66,10];
        ...
        us_win=frame.cs_start:frame.us_start-2;
        CR_ind_up=[];CR_ind_down=[];CS_response=cell(trial.acq_block_num+2,2);
        for ii=1:trial.acq_block_num+2
            ...
            aa=A_r_n(base_win,trial_ind,:);aa=squeeze(mean(aa,2,'omitnan'));
            %aa=A_r_n(base_win,trial_ind,:);aa=reshape(aa,length(base_win)*length(trial_ind),[]);
            m=mean(aa,1,'omitnan');sd=std(aa,[],1,'omitnan');
            baseline_up=m+ 1.5*sd; baseline_down=m- 1.5*sd;baseline_UR=m+1.5*sd;
            aa=A_r_n(:,ind,:);aaa=A_r(:,ind,:);
            for iscutmov=1 %1:no_cut;2:cut
                b=nan(length(us_win),length(ind),size(A,2));bb=nan(length(us_win),length(ind),size(A,2));
                for jj=1:length(ind)
                    b(:,jj,:)=aa(us_win,jj,:);
                    bb(:,jj,:)=aaa(us_win,jj,:);
                end
                m_CS=squeeze(mean(mean(b,1,'omitnan'),2,'omitnan'))';
                CS_response{ii,iscutmov}(:,:,:)=bb;
        ...
        %% part of plot from mapping_index_plot.m
            labels_typei={'CS-avtivation','CS-inhibition','US-activation','CS-up regulate','CS-down regulate','CS-stable regulate','US-up regulate','US-down regulate','US-stable regulate','CS-new emerged neuron','CS-up regulate-region','CS-down regulate-region','CS-stable regulate-region','US-up regulate-region','US-down regulate-region','US-stable regulate-region'};
        ...
        if exist('p_CS_up')==1
            if sum(isnan(p_CS_up(:,1)))<size(A,2);
                ranksumtest=0;
            else
                p_CS_up=nan(size(A,2),trial.acq_block_num+2,2);p_CS_down=nan(size(A,2),trial.acq_block_num+2,2);
                p_US_up=nan(size(A,2),trial.acq_block_num);p_US_down=nan(size(A,2),trial.acq_block_num);
                ranksumtest=1;
            end
        else
            p_CS_up=nan(size(A,2),2);p_CS_down=nan(size(A,2),2);
            p_US_up=nan(size(A,2),trial.acq_block_num);p_US_down=nan(size(A,2),trial.acq_block_num);
            ranksumtest=1;
        end
        ranksumtest=1;
        q_CS_up=p_CS_up;q_US_up=p_US_up;q_US_down=p_US_down;q_CS_down=p_CS_down;
        fdr_CS_up=p_CS_up;fdr_US_up=p_US_up;fdr_US_down=p_US_down;fdr_CS_down=p_CS_down;
        ...
        CS_up_ind_region=cell(trial.acq_block_num+2,2); CS_down_ind_region=cell(trial.acq_block_num+2,2); CS_stable_ind_region=cell(trial.acq_block_num+2,2);
        ...
        US_up_ind=cell(trial.acq_block_num+2,2); US_down_ind=cell(trial.acq_block_num+2,2); US_stable_ind=cell(trial.acq_block_num+2,2);
         US_up_ind_region=cell(trial.acq_block_num+2,2); US_down_ind_region=cell(trial.acq_block_num+2,2); US_stable_ind_region=cell(trial.acq_block_num+2,2);
        ...
        for iscutmov=1
            ...
            for ii=1:trial.acq_block_num+2
                switch ii
                    case 1
                        ...
                        leg='Pre';
                    case trial.acq_block_num+2
                        ind=find(CR_ind_up(:,end,iscutmov)==1);%ind=intersect(ind,CS_related_tst_ind);
                        leg='Post';
                    otherwise
                        ind=find(CR_ind_up(:,ii,iscutmov)==1);%ind=intersect(ind,CS_related_acq_ind{ii-1});
                        leg=['Acq.' num2str(ii)];
                end
            ...
            for ii=1:trial.acq_block_num+2
                switch ii
                    case 1
                        ...
                        leg='Pre';
                    case trial.acq_block_num+2
                        ind=find(CR_ind_up(:,end,iscutmov)==1);cr=squeeze(mean(mean(CS_response{end,iscutmov}(:,:,ind),1,'omitnan'),2,'omitnan'));%ind=intersect(ind,CS_related_tst_ind);
                        leg='Post';
                    otherwise
                        ind=find(CR_ind_up(:,ii,iscutmov)==1);cr=squeeze(mean(mean(CS_response{ii,iscutmov}(:,:,ind),1,'omitnan'),2,'omitnan'));%ind=intersect(ind,CS_related_acq_ind{ii-1});
                        leg=['Acq.' num2str(ii)];
                end
            ...
            for ii=1:trial.acq_block_num+2
                switch ii
                    case 1
                        ...
                        leg='Pre';
                    case trial.acq_block_num+2
                        ind=find(CR_ind_down(:,end,iscutmov)==1);%ind=intersect(ind,CS_related_tst_ind);
                        leg='Post';
                    otherwise
                        ind=find(CR_ind_down(:,ii,iscutmov)==1);%ind=intersect(ind,CS_related_acq_ind{ii-1});
                        leg=['Acq.' num2str(ii)];
                end
            ...
            for ii=1:trial.acq_block_num+2
                switch ii
                    case 1
                        ...
                        leg='Pre';
                    case trial.acq_block_num+2
                        ind=find(CR_ind_down(:,end,iscutmov)==1);cr=squeeze(mean(mean(CS_response{end,iscutmov}(:,:,ind),1,'omitnan'),2,'omitnan'));%ind=intersect(ind,CS_related_tst_ind);
                        leg='Post';
                    otherwise
                        ind=find(CR_ind_down(:,ii,iscutmov)==1);cr=squeeze(mean(mean(CS_response{ii,iscutmov}(:,:,ind),1,'omitnan'),2,'omitnan'));%ind=intersect(ind,CS_related_acq_ind{ii-1});
                        leg=['Acq.' num2str(ii)];
                end
                nexttile,mapback_colorcode_responses(supervoxel_raw(ind,:),cr);view(v(1),v(2));title(leg);
                set(gca,'fontsize',16)
            ...
            ind2=find(CR_ind_up(:,end,iscutmov)==1);
            ...
            ind=find(CR_ind_up(:,end,iscutmov)==1);
            %ind=intersect(ind,CS_related_tst_ind);
            subplot(1,3,2),mapback_colorcode_responses(supervoxel_raw(ind,:),squeeze(mean(mean(CS_response{end,1}(:,:,ind),1,'omitnan'),2,'omitnan')));title('post');view(v(1),v(2))
            subplot(1,3,3),mapback_colorcode_responses(supervoxel_raw(CS_new_emerged,:),squeeze(mean(mean(CS_response{end,1}(:,:,CS_new_emerged),1,'omitnan'),2,'omitnan')));title('newly emerged');view(v(1),v(2))
            ...
            %down/up-regulate
            if ranksumtest==1;
                x={CS_response{:,iscutmov}};
                for ii=1:size(A,2)
                    for jj=1:trial.acq_block_num+2
                        p_CS_up(ii,jj,iscutmov)=ranksum(squeeze(x{1}(:,:,ii)),squeeze(x{jj}(:,:,ii)),'tail','left');
                        p_CS_down(ii,jj,iscutmov)=ranksum(squeeze(x{1}(:,:,ii)),squeeze(x{jj}(:,:,ii)),'tail','right');
                    end
                end
                x=US_response;
                for ii=1:size(A,2)
                    for jj=1:trial.acq_block_num
                        p_US_up(ii,jj)=ranksum(squeeze(x{1}(:,:,ii)),squeeze(x{jj}(:,:,ii)),'tail','left');
                        p_US_down(ii,jj)=ranksum(squeeze(x{1}(:,:,ii)),squeeze(x{jj}(:,:,ii)),'tail','left');
                    end
                end
            end
            for jj=1:trial.acq_block_num+2
                [fdr_CS_up(:,jj,iscutmov),q_CS_up(:,jj,iscutmov)] = FDR_base(p_CS_up(:,jj,iscutmov),'BH');
                [fdr_CS_down(:,jj,iscutmov),q_CS_down(:,jj,iscutmov)] = FDR_base(p_CS_down(:,jj,iscutmov),'BH');
            end
            for jj=1:trial.acq_block_num
            ...
            CS_all=length(find(sum(CR_ind_up(:,:,iscutmov),2)>=1));
            ...
            for ii=1:trial.acq_block_num+2
                switch ii
                    ...
                    case 1
                        CS_up_ind{ii,iscutmov}=[];
                        CS_down_ind{ii,iscutmov}=[];
                        CS_stable_ind{ii,iscutmov}=[];
                        CS_up_ind_region{ii,iscutmov}=[];
                        CS_down_ind_region{ii,iscutmov}=[];
                        CS_stable_ind_region{ii,iscutmov}=[];
                        US_up_ind{ii,iscutmov}=[];
                        US_down_ind{ii,iscutmov}=[];
                        US_stable_ind{ii,iscutmov}=[];
                    otherwise
                        CS_up_ind{ii,iscutmov}=find(squeeze(p_CS_up(:,ii,iscutmov))<=0.05 & squeeze(sum(CR_ind_up(:,[1,ii],iscutmov),2))>=1);
                        CS_down_ind{ii,iscutmov}=find(squeeze(p_CS_down(:,ii,iscutmov))<=0.05 & squeeze(sum(CR_ind_up(:,[1,ii],iscutmov),2))>=1);
                        CS_stable_ind{ii,iscutmov}=find((squeeze(p_CS_up(:,ii,iscutmov))>0.05 & squeeze(p_CS_down(:,ii,iscutmov))>0.05) & squeeze(sum(CR_ind_up(:,[1,ii],iscutmov),2))>=1);
                        CS_up_ind_region{ii,iscutmov}=find(squeeze(p_CS_up(:,ii,iscutmov))<=0.05 );
                        CS_down_ind_region{ii,iscutmov}=find(squeeze(p_CS_down(:,ii,iscutmov))<=0.05 );
                        CS_stable_ind_region{ii,iscutmov}=find((squeeze(p_CS_up(:,ii,iscutmov))>0.05 & squeeze(p_CS_down(:,ii,iscutmov))>0.05));
                        US_up_ind{ii,iscutmov}=find(squeeze(p_US_up(:,ii-1))<=0.05 & squeeze(sum(UR_ind(:,[ii-1,1]),2))>=1);
                        US_down_ind{ii,iscutmov}=find(squeeze(p_US_down(:,ii-1))<=0.05 & squeeze(sum(UR_ind(:,[ii-1,1]),2))>=1);
                        US_stable_ind{ii,iscutmov}=find((squeeze(p_US_up(:,ii-1))>0.05 & squeeze(p_US_down(:,ii-1))>0.05) & squeeze(sum(UR_ind(:,[ii-1,1]),2))>=1);
                        US_up_ind_region{ii,iscutmov}=find(squeeze(p_US_up(:,ii-1))<=0.05);
                        US_down_ind_region{ii,iscutmov}=find(squeeze(p_US_down(:,ii-1))<=0.05);
                        US_stable_ind_region{ii,iscutmov}=find((squeeze(p_US_up(:,ii-1))>0.05 & squeeze(p_US_down(:,ii-1))>0.05));
                end
            ...
            for ii=1:trial.acq_block_num+2
                ...
                ind_all_CSUS_RESPONSIVE{1,ii,iscutmov}=find(CR_ind_up(:,ii,iscutmov)==1);
                ind_all_CSUS_RESPONSIVE{2,ii,iscutmov}=find(CR_ind_down(:,ii,iscutmov)==1);
                ...
                ind_all_CSUS_RESPONSIVE{11,ii,iscutmov}=CS_up_ind_region{ii,iscutmov};
                ind_all_CSUS_RESPONSIVE{12,ii,iscutmov}=CS_down_ind_region{ii,iscutmov};
                ind_all_CSUS_RESPONSIVE{13,ii,iscutmov}=CS_stable_ind_region{ii,iscutmov};
                ind_all_CSUS_RESPONSIVE{14,ii,iscutmov}=US_up_ind_region{ii,iscutmov};
                ind_all_CSUS_RESPONSIVE{15,ii,iscutmov}=US_down_ind_region{ii,iscutmov};
                ind_all_CSUS_RESPONSIVE{16,ii,iscutmov}=US_stable_ind_region{ii,iscutmov};
                ind=find(CR_ind_up(:,1,iscutmov)==1);
                ind2=find(CR_ind_down(:,1,iscutmov)==1);
                ind3=union(ind,ind2);
                ind3=setdiff(1:size(CR_ind_up(:,1,iscutmov),1),ind3);
            ...
            %% mapback
            CS_response_all={CS_response{:,iscutmov}};
            ...
            for typei=1:length((labels_typei))
                ...
                for sessioni=1:trial.acq_block_num+2
                    ...
                    if a~=0
                        ...
                        nexttile([1,n]);
                        imagesc(cr_trace(:,ind_type_sort{typei,sessioni,iscutmov})',[0 0.4]);colormap('hot');colorbar('Location','northoutside');hold on
    ...
    save(fullfile(p,'CR_ind_summary_align.mat'),'A_r','trial_hab','trial_test',...
        'CR_ind_up','CR_ind_down',...
        'UR_ind',...
        'corr_CS_up','corr_CS_down',...
        'CS_response','US_response',...
        ...
        'p_CS_up','p_CS_down','p_US_up','p_US_down',...
        'q_CS_up','q_CS_down','q_US_up','q_US_down',...
        'fdr_CS_up','fdr_CS_down','fdr_US_up','fdr_US_down',...
        'CS_up_ind','CS_down_ind','CS_stable_ind','CS_up','CS_down','CS_stable',...
        'US_up_ind','US_down_ind','US_stable_ind','US_up','US_down','US_stable',...
        'CS_up_ind_region','CS_down_ind_region','CS_stable_ind_region','US_up_ind_region','US_down_ind_region','US_stable_ind_region',...
