function US_mapping_singletrial_hbactivation(Path)
...
if 1%~(exist([fullfile(p,'singletrial'),'\CR_ind_summary_singletrial.mat'],'file')==2)
    ...
    if ~[exist(fullfile(p,'activities_aft_process.mat'),'file') && exist(fullfile(p,'para.mat'),'file') && exist(fullfile(p,'env.mat'),'file')]
        ...
        cs_win=frame.cs_start+2.4/fs.ca+1:frame.us_start-1;
        ...
        %         ind=find(CRon_activation_ind(:,12,1)==0);figure,plot(mean(A_r(:,12,ind),3));hold on;
        %         ind=find(CRon_activation_ind(:,12,1));plot(mean(A_r(:,12,ind),3));
        %% 有无CR off
