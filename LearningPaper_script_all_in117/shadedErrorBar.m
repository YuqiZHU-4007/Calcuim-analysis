...
params.addParameter('lineProps', '-k', @(x) ischar(x) | iscell(x) | ismatrix(x));
...
function H = makePlot(x,y,errBar,lineProps,transparent,patchSaturation)
    ...
    H.mainLine=plot(x,y,'color',lineProps{:},'linewidth',1.5);

    % Work out the color of the shaded region and associated lines.
    % Here we have the option of choosing alpha or a de-saturated
    % solid colour for the patch surface.
    if iscell(get(H.mainLine,'color'))
         mainLineColor=cell2mat(get(H.mainLine,'color'));
    else
        mainLineColor=get(H.mainLine,'color');
    end
    for ii=1:size(lineProps{:},1)
        H.mainLine(ii).Color=lineProps{:}(ii,:,:);
    end
    ...
    if length(size(errBar))>2
        uE=y+squeeze(errBar(:,:,1));
        lE=y-squeeze(errBar(:,:,2));
    else
        uE=y+errBar(1,:);
        lE=y-errBar(2,:);
    end
...
%         if length(size(errBar))>2
%             for ii=1:length(size(errBar))
%                 H.patch(ii)=patch(xP,yP(ii,:),1);
%                 set(H.patch(ii),'facecolor',patchColor(ii,:,:), ...
%                     'edgecolor','none', ...
%                     'facealpha',faceAlpha);
%                  %Make pretty edges around the patch.
%             H.edge(ii)=plot(x,lE(ii,:),'-','color',edgeColor(ii,:,:));
%             H.edge(ii)=plot(x,uE(ii,:),'-','color',edgeColor(ii,:,:));
%             end
%         else
